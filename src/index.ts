...
app.get("/logs/:serviceName/:jobName", async (req, res) => {
  try {
    const authorization = req.headers.authorization;
    const getRequest = new GetRequest<JobInfo[]>();
    const jobs: JobInfo[] = await getRequest.getRequest(
      pathLogs + req.params.serviceName + "/" + req.params.jobName,
      true,
      authorization
    );
    res.send(jobs);
...
app.post("/run/:serviceName", upload.single("file"), async (req, res) => {
  try {
    ...
    const service: Service = await getServiceRequest.getRequest(
      pathServices + req.params.serviceName,
      false,
      authorization
    );
